...
def generate_email_html(user_name, events):
    ...
    for event in events:
        ...
        processed_events.append({
            ...
            'time_str': time_str,
            'location': event.location if event.location else 'Local não especificado',
...
def send_email_reminder(token, user_email, user_name, events):
    ...
    result = call_graph_api(token, f"/users/{Config.ADMIN_EMAIL}/sendMail", method="POST", data=email_data)
    
    # Verifica o resultado do envio
    if result is None: # A API sendMail retorna None em caso de sucesso
...
def send_teams_message(token, user_id, user_name, events):
    ...
    message_result = call_graph_api(token, f"/chats/{chat_id}/messages", method="POST", data=message_data)
    ...
    if message_result and "id" in message_result:
        ...
        log_action(f"Falha ao enviar mensagem do Teams para o usuário {user_id}.", success=False)
        return False
...
def create_onedrive_file(token, user_id, user_name, events):
    ...
    try:
        ...
        log_action(f"Erro HTTP ao criar arquivo no OneDrive do usuário {user_id}: {e.response.status_code} - {e.response.text}", success=False)
        return False
    except requests.exceptions.RequestException as e:
        log_action(f"Erro de requisição ao criar arquivo no OneDrive do usuário {user_id}: {e}", success=False)
        return False
    except Exception as e:
        log_action(f"Erro inesperado ao criar arquivo no OneDrive do usuário {user_id}: {e}", success=False)
        return False
...
def send_admin_notification(subject, message):
    ...
    if not Config.ADMIN_EMAIL:
        log_action("ADMIN_EMAIL não configurado. Não é possível enviar notificação de erro.", success=False)
        return False
    ...
    if not all([smtp_server, smtp_username, smtp_password]):
        log_action("Configurações SMTP incompletas. Não é possível enviar notificação de erro por e-mail.", success=False)
        return False
    ...
    try:
        ...
        log_action(f"Notificação de erro enviada para o administrador ({Config.ADMIN_EMAIL}).")
        return True
    except Exception as e:
        log_action(f"Falha ao enviar notificação de erro para o administrador: {e}", success=False)
        return False
