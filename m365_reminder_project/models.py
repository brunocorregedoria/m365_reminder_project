...
class Event:
    def __init__(self, id, subject, body_preview, start_datetime, end_datetime, location, organizer, attendees, is_all_day):
        self.id = id
    ...
    def to_dict(self):
        return {
            ...
            "organizer": {"emailAddress": {"name": self.organizer.get("name"), "address": self.organizer.get("address")}} if self.organizer else None,
    ...
    def from_dict(cls, event_dict):
        start_dt = datetime.fromisoformat(event_dict["start"]["dateTime"].replace("Z", "+00:00"))
        end_dt = datetime.fromisoformat(event_dict["end"]["dateTime"].replace("Z", "+00:00"))
